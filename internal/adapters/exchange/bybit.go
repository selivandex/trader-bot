...
func NewBybitAdapter(apiKey, apiSecret string, testnet bool, cfg *config.ExchangeConfig) (*BybitAdapter, error) {
...
func (b *BybitAdapter) FetchFundingRate(ctx context.Context, symbol string) (float64, error) {
	...
	if ticker.Info != nil {
		...
		// Try float64 funding_rate
		if rate, ok := ticker.Info["funding_rate"].(float64); ok {
			return rate, nil
		}
		// Try string fundingRate and convert
		if rateStr, ok := ticker.Info["fundingRate"].(string); ok {
			var rate float64
...
func (b *BybitAdapter) CreateOrder(ctx context.Context, symbol string, orderType models.OrderType, side models.OrderSide, amount, price float64) (*models.Order, error) {
	...
	case models.TypeStopMarket:
		...
		params := map[string]interface{}{
			"stopPrice": price,
			"triggerBy": "LastPrice", // Can be "LastPrice", "MarkPrice", or "IndexPrice"
		}
	...
	case models.TypeTakeProfitMarket:
		// Take-profit market orders need takeProfit in params
		params := map[string]interface{}{
			"takeProfit": price,
			"triggerBy":  "LastPrice",
		}
	...
	case models.TypeTakeProfitLimit:
		// Take-profit limit orders need takeProfit and limit price
		params := map[string]interface{}{
			"takeProfit": price,
			"triggerBy":  "LastPrice",
		}
...
func (b *BybitAdapter) SetMarginMode(ctx context.Context, symbol string, marginMode string) error {
	// Validate margin mode - Bybit accepts "isolated" or "cross" (lowercase)
