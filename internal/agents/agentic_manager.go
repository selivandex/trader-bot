...
func NewAgenticManager(db *sqlx.DB, newsAggregator *news.Aggregator, aiProviders []ai.Provider) *AgenticManager {
...
// collectMarketData collects market data for agent
func (am *AgenticManager) collectMarketData(ctx context.Context, runner *AgenticRunner) (*models.MarketData, error) {
	...
	candlesMap := map[string][]models.Candle{"1h": candles}

	// Calculate technical indicators
	calc := indicators.NewCalculator()
	var technicalIndicators *models.TechnicalIndicators
	if len(candles) >= 26 {
		technicalIndicators, err = calc.Calculate(candles)
		if err != nil {
