...
CREATE TABLE IF NOT EXISTS collective_agent_memories (
    ...
    embedding FLOAT[] NOT NULL,         -- For semantic search
    ...
    CONSTRAINT check_collective_importance CHECK (importance >= 0 AND importance <= 1),
    CONSTRAINT check_collective_success_rate CHECK (success_rate >= 0 AND success_rate <= 1)
);
...
CREATE OR REPLACE VIEW best_collective_memories AS
SELECT 
    cm.id,
    cm.personality,
    cm.lesson,
    ...
    cm.created_at,
    CASE 
        WHEN cm.confirmation_count >= 10 AND cm.success_rate > 0.65 THEN 'proven'
        WHEN cm.confirmation_count >= 5 AND cm.success_rate > 0.55 THEN 'validated'
        WHEN cm.confirmation_count >= 2 AND cm.success_rate > 0.50 THEN 'emerging'
        ELSE 'unproven'
    END as reliability_tier
FROM collective_agent_memories cm
WHERE cm.confirmation_count >= 2
ORDER BY 
    cm.personality,
